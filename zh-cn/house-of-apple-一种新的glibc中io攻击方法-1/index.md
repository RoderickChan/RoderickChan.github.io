...
> 本文首发于[看雪论坛](https://bbs.pediy.com/thread-273418.htm)，仅在个人博客记录

分享一系列新的`glibc`中`IO`利用思路，暂且命名为`house of apple`。
这篇是`house of apple1`。
本站的`house of apple`系列文章的地址为：

- [house of apple1](https://roderickchan.cn/house-of-apple-%E4%B8%80%E7%A7%8D%E6%96%B0%E7%9A%84glibc%E4%B8%ADio%E6%94%BB%E5%87%BB%E6%96%B9%E6%B3%95-1/)
- [house of apple2](https://roderickchan.cn/house-of-apple-%E4%B8%80%E7%A7%8D%E6%96%B0%E7%9A%84glibc%E4%B8%ADio%E6%94%BB%E5%87%BB%E6%96%B9%E6%B3%95-2/)
- [house of apple3](https://roderickchan.cn/house-of-apple-%E4%B8%80%E7%A7%8D%E6%96%B0%E7%9A%84glibc%E4%B8%ADio%E6%94%BB%E5%87%BB%E6%96%B9%E6%B3%95-3/)
...
其中，`house of pig`除了需要劫持`IO_FILE`结构体，还需要劫持`tcache_perthread_struct`结构体或者能控制任意地址分配；`house of kiwi`则至少需要修改三个地方的值：`_IO_helper_jumps + 0xA0`和`_IO_helper_jumps + 0xA8`，另外还要劫持`_IO_file_jumps + 0x60`处的`_IO_file_sync`指针；而`house of emma`则至少需要修改两个地方的值，一个是`tls`结构体的`point_guard`(或者想办法泄露出来)，另外需要伪造一个`IO_FILE`或替换`vtable`为`xxx_cookie_jumps`的地址。
...
![image-20220615234316844](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_MHBA8T4S38HXFZ5.jpg)

**main**

`main`函数必须选一个`key`，大小在`6-10`。也就是说，分配的`chunk`都会属于`largebin`范围。

![image-20220615234353368](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_8EQHPG4TQB2SAZK.jpg)

**add**

限制了只能分配`key+0x10`、`key+0x20`、`2 * key + 0x10`大小的`chunk`

![image-20220615234649462](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_STJY78V984QSTDA.jpg)

**dele**

存在`UAF`，没有清空指针。

![image-20220615234744558](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_7MWTVUAY2J8U23K.jpg)

**read**

只给`1`次机会读。

![image-20220615234835069](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_6N5BQEWK5NCTF5E.jpg)

**write**

只给一次机会写，并只泄露出`0x10`个字节的数据。

![image-20220615234920427](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_GD4BKJ4KV3CJHBH.jpg)
...
![image-20220615235611606](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_U7UZA8NWCC4XG6R.jpg)
...
![img](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_6ECD7E3FD4WBJNX.jpg)
...
![image-20220616003350830](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_3MDBTS2UM4X22BT.jpg)
...
![image-20220616204035757](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_MW8U6CQNTPZVGE2.jpg)



然后使用`_IO_cookie_read`控制程序执行流：

![image-20220616204134860](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_QYAQCNH5JV9FG7Q.jpg)

成功劫持`rsp`：

![image-20220616204203067](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_NNFS86N9XDWJG6K.jpg)



接下来使用思路一，修改`tcache`变量，对于该变量的寻找同样可以使用`search`命令：

![image-20220616204339971](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_S4MFDNYXS8FPDE4.jpg)
...
![image-20220616204846469](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_BV6E4X7RAWU9Q4H.jpg)

然后分配到`_IO_str_jumps`：

![image-20220616205014138](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_8WGJCQN7YY45UV6.jpg)

后面的过程就一样了：

![image-20220616205139145](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_33P34CNTZ8GK36H.jpg)

最后成功输出`flag`:

![image-20220616205159794](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/956675_C9D2GTWT5JUAN88.jpg)
...
> URL: https://www.roderickchan.cn/zh-cn/house-of-apple-%E4%B8%80%E7%A7%8D%E6%96%B0%E7%9A%84glibc%E4%B8%ADio%E6%94%BB%E5%87%BB%E6%96%B9%E6%B3%95-1/  
