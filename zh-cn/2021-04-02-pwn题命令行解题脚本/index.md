...
- 本地调试
  - 开启`tmux`调试
  - 设置`gdb`断点，支持`函数名断点`、`地址断点`、`文件名:行号断点`、`$rebase(0x00)断点`
  - 设置`gdb script`，可以设置任何内置的`gdb`命令
- 远程答题
  - 默认支持`buuctf`，只需要指定远程`port`
  - 显式指定远程`ip`和`port`
- 使用的装饰器函数、偏函数
  - 在执行函数前、后线程休眠指定秒数
  - 记录函数执行日志
...
使用详情基本在注释里面写得很清楚了，这里展示一下输入`python exp.py -h`后显示的帮助信息：

![](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/20210402233749.png)

![](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/20210402233915.png)

![](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/20210402234008.png)
...
![](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/20210405125931.png)
自动下断点`malloc`和`free`
输入：`python3 exp.py ./ycb_2020_babypwn -t 1 -gb "\$rebase(0xc1f)"`，这里要加上转义字符，否会失败。
![](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/20210405133859.png)
的确断住了。
远程攻击：
输入：`python3 exp.py ./ycb_2020_babypwn -i "127.0.0.1" -p 6666`
![](https://lynne-markdown.oss-cn-hangzhou.aliyuncs.com/img/20210405134223.png)



### 参考与引用
- click帮助文档：<https://click.palletsprojects.com/en/7.x/> 
- pwntools帮助文档：<https://docs.pwntools.com/en/latest/>

---

> 作者: [roderick](https://www.roderickchan.cn)  
> URL: https://www.roderickchan.cn/zh-cn/2021-04-02-pwn%E9%A2%98%E5%91%BD%E4%BB%A4%E8%A1%8C%E8%A7%A3%E9%A2%98%E8%84%9A%E6%9C%AC/  
